...
function Initialize(cf::Float64)
    ...
    p::Float64 = 4.5
    M::Float64 = 1
    U::Array{Float64, 1} = ones(pars.ns) .* 0.5772 ./ pars.α
    V::Array{Float64, 1} = zeros(pars.ns, 2)
    Σ::Array{Float64, 2} = ones(pars.ns, 2) .* 0.5
    res = Results(μ, p, M, U, V, Σ, cf)
    pars, res
end

function FindU(pars, res, tol::Float64 = 1e-4)
    @unpack p, cf = res
    @unpack α, S, β, θ, F, ns = pars
    ...
    while err_u > tol
        ...
        for (i_s, s) in enumerate(S)
            ...
            Us = -0.5772 + log(sum(exp(α * V0 + 0.5772), exp(α * V1 + 0.5772)))
            U_cand[i_s] = (Us + 0.5772) / α
            res.Σ[i_s, 1] = exp(α*V0) / Us
            res.Σ[i_s, 2] = exp(α*V1) / Us
        end
        err_u = max(abs.(res.U .- U_cand))
        res.U = U_cand
    end
end
...
function StatDist(pars, res, tol_m::Float64 = 1e-4)
    ...
    while err_m > tol_m
        for i = 1:ns
            new_dist[i] = sum(Σ[:,1] .* F[:,i] .* μ) + M * sum(Σ[:,1] .* F[:,i] .* ν)
        end
