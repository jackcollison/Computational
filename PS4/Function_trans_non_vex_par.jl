...
#function for initializing model primitives and results
 function Initialize(θ::Float64, Zs::Array{Float64, 1}, γ::Float64, K1::Float64, K2::Float64, Vwor::Array{Float64, 3}, Vret::Array{Float64, 2}, Psiwor::Array{Float64, 3}, Psiret::Array{Float64, 2}, T::Int64, NT::Int64)
    ...
    r::Array{Float64} = ones(T) .* 0.025 #interest rate
    w::Array{Float64} = ones(T) .* 1.45 #wage
    b::Float64 = 0.2
        ...
        for a_index = 1:length_a_grid
         ...
         val_up = -Inf
            for ap_index = choice_lower:length_a_grid
               if (1+r[t]) * a + b > 0
                  ctmp_ret = (1+r[t]) * a + b - a_grid[ap_index]
               ...
               elseif (1+r[t]) * a + b <= 0
                  ...
                  res.pol_func_ret_tran[a_index,j,t] = a_grid[1]
               end
            end
        end
    end
...
function Bellman_wor(prim::Primitives, res::Results, t::Int64)
        ...
        choice_lower = 1
          for i = 1:length_a_grid
           ...
           for ip = choice_lower:length_a_grid
             ltmp_wor = min(1, max((γ * (1 - θ) * ef[Jr-1] * Zs[j] * w[t] - (1-γ) * ((1+r[t]) * a - a_grid[ip])) / ((1-θ) * w[t] * ef[Jr-1] * Zs[j]), 0))
             ctmp_wor = w[t] * (1 - θ) * ef[Jr-1] * Zs[j] * ltmp_wor + (1 + r[t]) * a - a_grid[ip]
             ...
             if vtmp_wor < val_up
                ...
                res.lab_func_wor_tran[i,Jr-1,j,t] = min(1, max((γ * (1 - θ) * ef[Jr-1] * Zs[j]* w[t] - (1-γ)* ((1+r[t]) * a - a_grid[ip-1])) / ((1-θ) * w[t] * ef[Jr-1] * Zs[j]), 0))
                choice_lower = ip - 1
                break
             elseif ip == length_a_grid
                next_val[i,Jr-1,j] = vtmp_wor
                res.pol_func_wor_tran[i,Jr-1,j,t] = a_grid[ip]
                res.lab_func_wor_tran[i,Jr-1,j,t] = min(1, max((γ * (1 - θ) * ef[Jr-1] * Zs[j] * w[t] - (1-γ) * ((1+r[t]) * a - a_grid[ip])) / ((1-θ) * w[t] * ef[Jr-1] * Zs[j]), 0))
             end
             val_up = vtmp_wor
          end
       end
      ...
      for k = 1:2
         choice_lower = 1
           for i = 1:length_a_grid
            ...
            val_up = -Inf
            for ip = choice_lower:length_a_grid
               ltmp_wor= min(1, max((γ * (1 - θ) * ef[j] * Zs[k] * w[t] - (1-γ) * ((1+r[t]) * a - a_grid[ip])) / ((1-θ) * w[t] * ef[j] * Zs[k]), 0))
               ctmp_wor = w[t] * (1 - θ) * ef[j] * Zs[k] * ltmp_wor + (1 + r[t]) * a - a_grid[ip]
               ctmp_wor = ifelse(ctmp_wor> 0, 1, 0)*ctmp_wor
               ...
               if vtmp_wor < val_up
                  ...
                  res.lab_func_wor_tran[i,j,k,t] = min(1, max((γ * (1 - θ) * ef[j] * Zs[k] * w[t] - (1-γ) * ((1+r[t]) * a - a_grid[ip-1])) / ((1-θ) * w[t] * ef[j] * Zs[k]), 0))
                  choice_lower = ip - 1
                  break
               elseif ip == length_a_grid
                  next_val[i,j,k] = vtmp_wor
                  res.pol_func_wor_tran[i,j,k,t] = a_grid[ip]
                  res.lab_func_wor_tran[i,j,k,t] = min(1, max((γ * (1 - θ) * ef[j] * Zs[k] * w[t] - (1-γ) * ((1+r[t]) * a - a_grid[ip])) / ((1-θ) * w[t] * ef[j] * Zs[k]), 0))
               end
               val_up = vtmp_wor
            end
        end
 ...
 function get_trans(prim::Primitives, res::Results, tol::Float64=1e-4, err::Float64=100.0)
    ...
    while err > tol
        ...
        res.r = α .* res.Ks[1:T].^(α-1) .* res.Ls.^(1-α) .- δ
        res.w = (1-α) .* res.Ks[1:T].^α .* res.Ls.^(-α)
       ...
       for t in 1:T
        ...
        errL = maximum(abs.(Ls_new - Ls_old)/Ls_old)
        err = errK + errL
    ...
    #Finalize with the values of L and K
    res.val_func_ret_tran = res.val_ter_ret # Put V_T as the terminal value function to begin the iteration backwards
    res.val_func_wor_tran = res.val_ter_wor # Put V_T as the terminal value function to begin the iteration backwards
    res.r = α .* res.Ks[1:T].^(α-1) .* res.Ls.^(1-α) .- δ
    res.w = (1-α) .* res.Ks[1:T].^α .* res.Ls.^(-α)
