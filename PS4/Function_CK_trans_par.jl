...
# T operator
@everywhere function Bellman_ret(prim::Primitives,res::Results)
    @unpack val_func_ret, r, γ, b = res #unpack value function
...
@everywhere function Bellman_wor(prim::Primitives, res::Results)
...
@everywhere function get_dist(prim::Primitives, res::Results)
...
@everywhere function get_prices(prim::Primitives, res::Results; tol::Float64 = 1e-4, err::Float64 = 100.0)
    @unpack α, δ, a_grid, length_a_grid, Π, Jr, N, ef, mu = prim
    @unpack Zs, θ = res
    ...
    while abs(ED) > tol
        ...
        Bellman_ret(prim, res)
        Bellman_wor(prim, res)
